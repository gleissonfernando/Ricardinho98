...
async function showReport(interaction: StringSelectMenuInteraction | ButtonInteraction, context: BotContext, reportId: string) { if (!interaction.deferred && !interaction.replied) await interaction.deferReply({ ephemeral: true }); const data = await context.api.getPolicePatrolReport(reportId); if (!(await canViewReport(interaction, context, data.report))) { await interaction.editReply("Você não possui permissão para visualizar este relatório."); return; } const text = reportText(data.report, data.messages); const buttons = new ActionRowBuilder<ButtonBuilder>().addComponents(new ButtonBuilder().setCustomId(`${PREFIX}:pdf:${reportId}`).setEmoji("📄").setLabel("Exportar PDF").setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId(`${PREFIX}:html:${reportId}`).setEmoji("🌐").setLabel("Exportar HTML").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`${PREFIX}:json:${reportId}`).setEmoji("🧩").setLabel("Exportar JSON").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId(`${PREFIX}:delete:${reportId}`).setEmoji("🗑️").setLabel("Excluir").setStyle(ButtonStyle.Danger)); await interaction.editReply({ components: [{ type: 17, accent_color: 0x2563eb, components: [{ type: 10, content: text.slice(0, 3900) }, buttons] }], flags: MessageFlags.IsComponentsV2 }); }
...
function pendingLogPayload(guildName: string, report: PolicePatrolReport, messages: PolicePatrolMessage[], image: PanelVisualConfig | null) {
  const actions = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`${PREFIX}:approve:${report.id}`).setEmoji("✅").setLabel("Aprovar relatorio").setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`${PREFIX}:reject:${report.id}`).setEmoji("❌").setLabel("Reprovar relatorio").setStyle(ButtonStyle.Danger),
    new ButtonBuilder().setCustomId(`${PREFIX}:view:${report.id}`).setEmoji("👁️").setLabel("Ver completo").setStyle(ButtonStyle.Secondary)
  );
...
function evaluationLogPayload(report: PolicePatrolReport) {
  ...
  const actions = new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(`${PREFIX}:approve:${report.id}`).setEmoji("✅").setLabel("Aprovar relatorio").setStyle(ButtonStyle.Success).setDisabled(true),
    new ButtonBuilder().setCustomId(`${PREFIX}:reject:${report.id}`).setEmoji("❌").setLabel("Reprovar relatorio").setStyle(ButtonStyle.Danger).setDisabled(true),
    new ButtonBuilder().setCustomId(`${PREFIX}:view:${report.id}`).setEmoji("👁️").setLabel("Ver completo").setStyle(ButtonStyle.Secondary)
  );
