...
export type FivemActionDefinition = { id: string; architecture: FivemActionArchitecture; name: string; description: string; emoji: string | null; imageUrl: string | null; bannerUrl: string | null; color: string; authorizedRoleIds: string[]; destinationSystem: string | null; maxParticipants: number; allowedTypes: FivemActionType[]; enabled: boolean; order: number };
export type FivemActionParticipant = { userId: string; username: string; roleIds: string[]; joinedAt: string; leftAt: string | null };
export type FivemActionSession = { id: string; botId: string; guildId: string; architecture: FivemActionArchitecture; actionId: string; actionName: string; actionDescription: string; actionEmoji: string | null; actionImageUrl: string | null; actionColor: string; actionType: FivemActionType; openerId: string; openerName: string; channelId: string | null; messageId: string | null; status: "active" | "victory" | "defeat" | "cancelled"; result: "victory" | "defeat" | null; observation: string | null; extraInfo: string | null; finalParticipantCount: number | null; maxParticipants: number; participants: FivemActionParticipant[]; startedAt: string; finishedAt: string | null };
...
export type DmSettings = { id: string; botId: string; guildId: string; enabled: boolean; authorizedRoleIds: string[]; logChannelId: string | null; bannerUrl: string | null; imageUrl: string | null; imagePosition: DmImagePosition; color: string; teamName: string; defaultTitle: string; defaultText: string; footerText: string | null; buttons: DmButtonConfig[]; blockBots: boolean; saveContentInLogs: boolean };
export type SummonsCompetence = "iab" | "conselho" | "hcmd" | "comissario";
export type SummonsSettings = { id: string; botId: string; guildId: string; enabled: boolean; categoryId: string | null; temporaryCategoryId: string | null; authorizedRoleIds: string[]; moderatorRoleIds: string[]; anonymityEnabled: boolean; teamRoleIds: string[]; teamMentionRoleIds: string[]; conselhoRoleIds: string[]; conselhoMentionRoleIds: string[]; hcmdRoleIds: string[]; hcmdMentionRoleIds: string[]; comissarioRoleIds: string[]; comissarioMentionRoleIds: string[]; allowedCommandRoleIds: string[]; iabCategoryId: string | null; conselhoCategoryId: string | null; hcmdCategoryId: string | null; comissarioCategoryId: string | null; iabLogChannelId: string | null; conselhoLogChannelId: string | null; hcmdLogChannelId: string | null; comissarioLogChannelId: string | null; panelBannerUrl: string | null; defaultDeadline: string | null; teamAvatarUrl: string | null; privateLogChannelId: string | null; logChannelId: string | null; bannerUrl: string | null; color: string; publicResponsibleName: string; dmTitle: string; dmDescription: string; dmButtonText: string; defaultMessage: string; deleteDelaySeconds: number; transcriptEnabled: boolean };
export type SummonsRecord = { id: string; botId: string; guildId: string; targetId: string; requesterId: string; reason: string; notes: string | null; channelId: string | null; panelMessageId: string | null; dmMessageId: string | null; dmDeliveryStatus: "pending" | "sent" | "failed"; dmDeliveryError: string | null; settingsSnapshot: Record<string, unknown>; status: "creating" | "active" | "closing" | "closed" | "failed"; transcript: string | null; createdAt: string; closedAt: string | null; closedBy: string | null; deleteAt: string | null; updatedAt: string };
...
export type PoliceCourseConfig = { id: string; botId: string; guildId: string; enabled: boolean; logChannelId: string | null; approvalChannelId: string | null; certificateChannelId: string | null; notificationChannelId: string | null; defaultCategoryId: string | null; defaultPanelChannelId: string | null; generalManagerUserIds: string[]; allowedManagerRoles: string[]; allowedFinishRoles: string[]; createRoleIds: string[]; editRoleIds: string[]; deleteRoleIds: string[]; approveRoleIds: string[]; cancelRoleIds: string[]; concludeRoleIds: string[]; allowJoinAfterStart: boolean; allowLeaveAfterStart: boolean; dmOnFinish: boolean; dmOnCancel: boolean; lockChannelOnFinish: boolean; lockChannelOnCancel: boolean; deletePanelOnCancel: boolean; removeDepartedMembers: boolean; panelHeader: string; panelText: string; accentColor: string; joinButtonStyle: "primary" | "secondary" | "success" | "danger"; leaveButtonStyle: "primary" | "secondary" | "success" | "danger"; createdAt: string; updatedAt: string };
...
export class ApiClient {
  ...
  async updatePoliceRhPanelState(guildId: string, messageId: string | null) {
    const { data } = await this.http.post<{ module: { config: Record<string, unknown> } }>(
      `/bot/runtime/guilds/${encodeURIComponent(guildId)}/police-rh/panel-state`,
      { messageId },
      { timeout: 8_000 }
    );
    return data.module;
  ...
  async evaluatePolicePatrolReport(reportId: string, actorId: string, result: "approved" | "rejected") { const { data } = await this.http.post<{ report: PolicePatrolReport }>(`/police-patrol-reports/bot/reports/${reportId}/evaluate`, { actorId, result }); return data.report; }
  ...
  async createFivemActionSession(input: { guildId: string; architecture: FivemActionArchitecture; actionId: string; actionType: FivemActionType; openerId: string; openerName: string; openerRoleIds?: string[] }) {
  ...
  async finishFivemActionSession(sessionId: string, input: { actorId: string; actorRoleIds: string[]; actorIsAdmin: boolean; result: "victory" | "defeat"; observation?: string | null; extraInfo?: string | null; finalParticipantCount?: number | null }) { const { data } = await this.http.post<{ session: FivemActionSession }>(`/fivem-actions/bot/sessions/${sessionId}/finish`, input); return data.session; }
  async cancelFivemActionSession(sessionId: string, input: { actorId: string; actorRoleIds: string[]; actorIsAdmin: boolean; reason?: string | null }) { const { data } = await this.http.post<{ session: FivemActionSession }>(`/fivem-actions/bot/sessions/${sessionId}/cancel`, input); return data.session; }

  async getDmSettings(guildId: string) { const { data } = await this.http.get<{ settings: DmSettings }>(`/communication/bot/dm/${guildId}`); return data.settings; }
  async saveDmSettings(guildId: string, input: Partial<DmSettings>) { const { data } = await this.http.patch<{ settings: DmSettings }>(`/communication/bot/dm/${guildId}`, input); return data.settings; }
  async recordDm(input: { guildId: string; senderId: string; targetId: string; title: string; description: string; hasImage?: boolean; button: DmButtonConfig | null; status: "sent" | "failed"; error?: string | null }) { const { data } = await this.http.post("/communication/bot/dm/log", input); return data.log; }
  async getSummonsSettings(guildId: string) { const { data } = await this.http.get<{ settings: SummonsSettings }>(`/communication/bot/summons/${guildId}`); return data.settings; }
  async saveSummonsSettings(guildId: string, input: Partial<SummonsSettings>) { const { data } = await this.http.patch<{ settings: SummonsSettings }>(`/communication/bot/summons/${guildId}`, input); return data.settings; }
  async createSummons(input: { guildId: string; targetId: string; requesterId: string; reason: string; notes?: string | null; settingsSnapshot?: Record<string, unknown> }) { const { data } = await this.http.post<{ summons: SummonsRecord }>("/communication/bot/summons", input); return data.summons; }
  async getSummons(id: string) { const { data } = await this.http.get<{ summons: SummonsRecord }>(`/communication/bot/summons/item/${id}`); return data.summons; }
  async getSummonsByChannel(channelId: string) { const { data } = await this.http.get<{ summons: SummonsRecord | null }>(`/communication/bot/summons/channel/${channelId}`); return data.summons; }
  async updateSummons(id: string, input: Partial<SummonsRecord>) { const { data } = await this.http.patch<{ summons: SummonsRecord }>(`/communication/bot/summons/item/${id}`, input); return data.summons; }

  async getOpenPointSettings(guildId: string) { const { data } = await this.http.get<{ settings: OpenPointSettings }>(`/open-point-notifications/bot/${guildId}/settings`); return data.settings; }
  async saveOpenPointSettings(guildId: string, input: Partial<OpenPointSettings>) { const { data } = await this.http.patch<{ settings: OpenPointSettings }>(`/open-point-notifications/bot/${guildId}/settings`, input); return data.settings; }
  async notifyOpenPoint(guildId: string, input: { appliedBy: string; userId: string }) { const { data } = await this.http.post<{ counter: OpenPointCounter }>(`/open-point-notifications/bot/${guildId}/notify`, input); return data.counter; }
  async getOpenPointCounter(guildId: string, userId: string) { const { data } = await this.http.get<{ counter: OpenPointCounter }>(`/open-point-notifications/bot/${guildId}/counters/${userId}`); return data.counter; }
  async resetOpenPointCounter(guildId: string, userId: string, resetBy: string) { const { data } = await this.http.post<{ counter: OpenPointCounter }>(`/open-point-notifications/bot/${guildId}/counters/${userId}/reset`, { resetBy }); return data.counter; }
  async markOpenPointFineGenerated(guildId: string, userId: string) { const { data } = await this.http.post<{ counter: OpenPointCounter }>(`/open-point-notifications/bot/${guildId}/counters/${userId}/fine-generated`); return data.counter; }
  ...
  async beginPoliceCourse(guildId: string, courseId: string, actorId: string) {
    const { data } = await this.http.post<{ course: PoliceCourse }>(`/police-courses/bot/${guildId}/courses/${courseId}/begin`, { actorId });
    return data.course;
  }

  async startPoliceCourse(guildId: string, courseId: string, input: { instructorId: string; instructorName: string; date?: string; time: string; maxSlots: number; location: string; actorId: string }) {
