...
export type DmSettings = { id: string; botId: string; guildId: string; enabled: boolean; authorizedRoleIds: string[]; logChannelId: string | null; bannerUrl: string | null; imageUrl: string | null; color: string; defaultTitle: string; defaultText: string; footerText: string | null; buttons: DmButtonConfig[]; blockBots: boolean };
export type SummonsSettings = { id: string; botId: string; guildId: string; enabled: boolean; categoryId: string | null; temporaryCategoryId: string | null; authorizedRoleIds: string[]; moderatorRoleIds: string[]; logChannelId: string | null; bannerUrl: string | null; color: string; defaultMessage: string; deleteDelaySeconds: number; transcriptEnabled: boolean };
export type SummonsRecord = { id: string; botId: string; guildId: string; targetId: string; requesterId: string; reason: string; notes: string | null; channelId: string | null; panelMessageId: string | null; status: "creating" | "active" | "closing" | "closed" | "failed"; transcript: string | null; createdAt: string; closedAt: string | null; closedBy: string | null; deleteAt: string | null; updatedAt: string };
...
export class ApiClient {
  ...
  async createSummons(input: { guildId: string; targetId: string; requesterId: string; reason: string; notes?: string | null }) { const { data } = await this.http.post<{ summons: SummonsRecord }>("/communication/bot/summons", input); return data.summons; }
