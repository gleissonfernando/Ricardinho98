...
const policeFlightConfigSchema = z.object({
  ...
  dafRoleIds: z.array(snowflakeSchema).max(100).default([]),
  ...
  descriptionText: z.string().trim().max(1200).default("Use Entrar na Escalacao para escolher Piloto ou Atirador.\nUse Sair da Escalacao para liberar apenas a sua vaga.\n\nO encerramento oficial deve ser feito por um responsavel autorizado."),
  panelFooter: z.string().trim().max(200).default(""),
  panelImage: z.string().trim().max(500).nullable().default(null),
  ...
  enterPilotButtonText: z.string().trim().min(1).max(80).default("Entrar como Piloto"),
  enterShooterButtonText: z.string().trim().min(1).max(80).default("Entrar como Atirador"),
  enterButtonText: z.string().trim().min(1).max(80).default("Entrar na Escalacao"),
  leaveButtonText: z.string().trim().min(1).max(80).default("Sair da Escalacao"),
  closeButtonText: z.string().trim().min(1).max(80).default("Encerrar Escalacao"),
  enterButtonEmoji: z.string().trim().max(40).default("✈️"),
  leaveButtonEmoji: z.string().trim().max(40).default("🚪"),
  closeButtonEmoji: z.string().trim().max(40).default("🔒"),
  embedColor: z.string().regex(/^#[0-9a-f]{6}$/i).default("#3b82f6"),
  allowSameUserBothFunctions: z.boolean().default(false),
  allowReplaceOccupiedRole: z.boolean().default(false),
  scaleId: z.coerce.number().int().min(1).max(1_000_000).default(1),
...
const policeRhConfigSchema = z.object({
  ...
  adornoTitle: z.string().trim().min(1).max(120).default("🎖️ Sistema de Adornos"),
  adornoDescription: z.string().trim().max(1200).default("🎖️ Solicite análise de adorno com tipo, descrição, motivo e imagem."),
  adornoImageUrl: z.string().trim().max(2048).default(""),
  adornoBannerUrl: z.string().trim().max(2048).default(""),
  adornoFooterText: z.string().trim().max(200).default("🎖️ Solicitação de adorno"),
  ...
  adornoDmRejectedMessage: z.string().trim().max(1200).default("❌ Sua solicitação de adorno foi recusada."),
  adornoDmMessage: z.string().trim().max(1200).default("🎖️ Sua solicitação de adorno foi atualizada pela equipe.")
});
