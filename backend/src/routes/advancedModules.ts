...
const policeRhConfigSchema = z.object({
  ...
  adornoTitle: z.string().trim().min(1).max(120).default("North Police Department"),
  adornoDescription: z.string().trim().max(1200).default(""),
  adornoImageUrl: z.string().trim().max(2048).default(""),
  adornoBannerUrl: z.string().trim().max(2048).default(""),
  adornoFooterText: z.string().trim().max(200).default("Solicitação enviada ao HCMD"),
...
function normalizeModuleConfig(moduleId: z.infer<typeof moduleIdSchema>, config: Record<string, unknown>) {
  ...
  if (moduleId === "police-reports") {
    ...
    return {
      ...
      responsibleRoleId,
      responsibleRoleIds: parsed.responsibleRoleIds
    };
  }
