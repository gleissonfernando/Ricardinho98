...
const settingsSchema = z.object({
  ...
  logChannelId: snowflake.nullable().optional(), logChannelIds: z.array(snowflake).max(100).optional(),
  historyChannelId: snowflake.nullable().optional(), historyChannelIds: z.array(snowflake).max(100).optional(),
  createRoleIds: z.array(snowflake).max(50).optional(), finishRoleIds: z.array(snowflake).max(50).optional(),
  statsRoleIds: z.array(snowflake).max(50).optional(), configRoleIds: z.array(snowflake).max(50).optional(),
...
fivemActionsRouter.post("/bot/sessions", requireBot, async (req, res, next) => { try { const botId = await botRuntimeId(req); const input = z.object({ guildId: snowflake, architecture: architectureSchema, actionId: z.string().uuid(), actionType: z.enum(["fuga", "tiro"]), openerId: snowflake, openerName: z.string().max(100), openerRoleIds: z.array(snowflake).max(100).default([]) }).parse(req.body); await licensed(botId, input.architecture); res.status(201).json({ session: await createFivemActionSession({ botId, ...input }) }); } catch (error) { next(error); } });
...
fivemActionsRouter.post("/bot/sessions/:sessionId/finish", requireBot, async (req, res, next) => { try { const botId = await botRuntimeId(req); await requireLicensedSession(botId, req.params.sessionId!); const input = z.object({ actorId: snowflake, actorRoleIds: z.array(snowflake).max(100).default([]), actorIsAdmin: z.boolean().default(false), result: z.enum(["victory", "defeat"]), observation: z.string().max(1000).nullable().optional(), extraInfo: z.string().max(1000).nullable().optional(), finalParticipantCount: z.coerce.number().int().min(0).max(100).nullable().optional() }).parse(req.body); res.json({ session: await finishFivemActionSession(botId, req.params.sessionId!, input.actorId, input.result, input) }); } catch (error) { next(error); } });
fivemActionsRouter.post("/bot/sessions/:sessionId/cancel", requireBot, async (req, res, next) => { try { const botId = await botRuntimeId(req); await requireLicensedSession(botId, req.params.sessionId!); const input = z.object({ actorId: snowflake, actorRoleIds: z.array(snowflake).max(100).default([]), actorIsAdmin: z.boolean().default(false), reason: z.string().max(1000).nullable().optional() }).parse(req.body); res.json({ session: await cancelFivemActionSession(botId, req.params.sessionId!, input.actorId, input) }); } catch (error) { next(error); } });
