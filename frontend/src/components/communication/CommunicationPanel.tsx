    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      <Card><CardHeader><CardTitle>Configuração</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        ...
        {type === "dm" ? <>
          ...
          <label className="md:col-span-2 text-sm">Nome público do órgão<input maxLength={80} className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" placeholder="Equipe IAB" value={settings.publicResponsibleName ?? "Equipe IAB"} disabled={!canManage} onChange={(event) => patch({ publicResponsibleName: event.target.value || "Equipe IAB" })} /></label>
