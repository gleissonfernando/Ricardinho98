...
export function CommunicationPanel({ type, botId, guild, canManage }: { type: "dm" | "summons"; botId?: string | null; guild: DashboardGuild | null; canManage: boolean }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      ...
      <Card><CardHeader><CardTitle>Preview</CardTitle></CardHeader><CardContent>{type === "dm" ? <DmPreview guildName={guild.name} settings={settings} /> : <div className="overflow-hidden rounded-md border border-zinc-700 bg-[#313338]" style={{ borderLeftColor: settings.color, borderLeftWidth: 4 }}>{settings.bannerUrl ? <img src={settings.bannerUrl} className="aspect-[16/6] w-full object-cover" /> : null}<div className="p-4"><p className="text-sm font-semibold text-white">🔔 Intimação em andamento</p><p className="mt-2 whitespace-pre-wrap text-sm text-zinc-300">{settings.defaultMessage}</p><button className="mt-4 rounded bg-red-600 px-3 py-2 text-sm text-white">Finalizar Intimação</button></div></div>}</CardContent></Card>
    </div>
