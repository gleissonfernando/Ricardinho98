    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      <Card><CardHeader><CardTitle>Configuração</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        ...
        {type === "dm" ? <>
          ...
          <label className="text-sm">Nome público do responsável<input maxLength={80} className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" value={settings.publicResponsibleName} disabled={!canManage} onChange={(event) => patch({ publicResponsibleName: event.target.value })} /></label>
          <label className="text-sm">Título da DM<input maxLength={100} className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" value={settings.dmTitle} disabled={!canManage} onChange={(event) => patch({ dmTitle: event.target.value })} /></label>
          <label className="text-sm">Texto do botão da DM<input maxLength={80} className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" value={settings.dmButtonText} disabled={!canManage} onChange={(event) => patch({ dmButtonText: event.target.value })} /></label>
          <label className="md:col-span-2 text-sm">Descrição da DM<textarea maxLength={1000} className="mt-2 min-h-24 w-full rounded-md border border-zinc-800 bg-black p-3" value={settings.dmDescription} disabled={!canManage} onChange={(event) => patch({ dmDescription: event.target.value })} /></label>
          ...
          <label className="md:col-span-2 text-sm">Mensagem do painel no canal temporário<textarea className="mt-2 min-h-24 w-full rounded-md border border-zinc-800 bg-black p-3" value={settings.defaultMessage} disabled={!canManage} onChange={(event) => patch({ defaultMessage: event.target.value })} /></label>
...
function SummonsPreview({ settings }: { settings: any }) {
  return <div className="space-y-4">
    <div className="overflow-hidden rounded-md border border-zinc-700 bg-[#313338]" style={{ borderLeftColor: settings.color, borderLeftWidth: 4 }}>
      ...
      <div className="p-4"><p className="font-semibold text-white">{settings.dmTitle}</p><p className="mt-2 whitespace-pre-wrap text-sm text-zinc-300">{settings.dmDescription}</p><p className="mt-3 text-sm text-zinc-300"><b>Canal:</b> #intimacao-usuario</p><p className="text-sm text-zinc-300"><b>Responsável:</b> {settings.publicResponsibleName}</p><button className="mt-4 rounded bg-[#5865f2] px-3 py-2 text-sm text-white">{settings.dmButtonText}</button></div>
    </div>
    <div className="rounded-md border border-zinc-700 bg-[#313338] p-4" style={{ borderLeftColor: settings.color, borderLeftWidth: 4 }}><p className="font-semibold text-white">🔔 Intimação aberta</p><p className="mt-2 text-sm text-zinc-300">{settings.defaultMessage}</p><p className="mt-3 text-sm text-zinc-300"><b>Intimado:</b> @usuário</p><p className="text-sm text-zinc-300"><b>Status:</b> Aguardando resposta</p><p className="text-sm text-zinc-300"><b>Responsável:</b> {settings.publicResponsibleName}</p></div>
  </div>;
}
