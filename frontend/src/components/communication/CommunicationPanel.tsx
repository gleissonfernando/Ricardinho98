...
export function CommunicationPanel({ type, botId, guild, canManage }: { type: "dm" | "summons"; botId?: string | null; guild: DashboardGuild | null; canManage: boolean }) {
  ...
  const upload = async (file: File, target: "bannerUrl" | "imageUrl" = "bannerUrl") => { setBusy(true); try { const result = await uploadPanelImage(guild.id, type === "dm" ? "dm-system" : "summons-system", file, botId); patch({ [target]: result.imageUrl }); setMessage(`${target === "bannerUrl" ? "Banner" : "Imagem"} enviado. Salve para aplicar.`); } catch (error) { setMessage(readMessage(error)); } finally { setBusy(false); } };
  const dmButton = type === "dm" ? settings.buttons?.[0] ?? { id: "default", label: "", style: "link", url: null } : null;
  const patchDmButton = (value: Partial<{ label: string; url: string | null }>) => {
    const label = value.label ?? dmButton?.label ?? "";
    const url = value.url ?? dmButton?.url ?? null;
    patch({ buttons: label || url ? [{ id: "default", style: "link", label, url }] : [] });
  };
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
      <Card><CardHeader><CardTitle>Configuração</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2">
        ...
        {type === "dm" ? <>
          ...
          <label className="text-sm">Texto padrão do botão<input className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" value={dmButton?.label ?? ""} disabled={!canManage} onChange={(event) => patchDmButton({ label: event.target.value })} /></label>
          <label className="text-sm">URL padrão do botão<input className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" placeholder="https://" value={dmButton?.url ?? ""} disabled={!canManage} onChange={(event) => patchDmButton({ url: event.target.value || null })} /></label>
          <label className="text-sm">Banner padrão<input className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" placeholder="https://" value={settings.bannerUrl ?? ""} disabled={!canManage} onChange={(event) => patch({ bannerUrl: event.target.value || null })} /></label>
          <label className="text-sm">Imagem do painel<input className="mt-2 h-11 w-full rounded-md border border-zinc-800 bg-black px-3" placeholder="https://" value={settings.imageUrl ?? ""} disabled={!canManage} onChange={(event) => patch({ imageUrl: event.target.value || null })} /></label>
        </> : <>
        ...
        {type === "dm" ? <FivemResourceMultiSelect label="Cargos que podem usar /dm" options={roles} values={settings.authorizedRoleIds} disabled={!canManage} onChange={(authorizedRoleIds) => patch({ authorizedRoleIds })} /> : <RolePicker label="Cargos autorizados" roles={roles} value={settings.authorizedRoleIds} disabled={!canManage} onChange={(authorizedRoleIds) => patch({ authorizedRoleIds })} />}
        ...
        <label className="text-sm">Upload de banner<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="mt-2 block w-full rounded-md border border-zinc-800 bg-black p-2" disabled={!canManage || busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file, "bannerUrl"); }} /></label>
        {type === "dm" ? <label className="text-sm">Upload de imagem do painel<input type="file" accept="image/png,image/jpeg,image/webp,image/gif" className="mt-2 block w-full rounded-md border border-zinc-800 bg-black p-2" disabled={!canManage || busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file, "imageUrl"); }} /></label> : null}
      ...
      <Card><CardHeader><CardTitle>Preview</CardTitle></CardHeader><CardContent><div className="overflow-hidden rounded-md border border-zinc-700 bg-[#313338]" style={{ borderLeftColor: settings.color, borderLeftWidth: 4 }}>{settings.bannerUrl ? <img src={settings.bannerUrl} className="aspect-[16/6] w-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : null}<div className="p-4"><p className="text-sm font-semibold text-white">{type === "dm" ? "📩 Nova mensagem da equipe" : "🔔 Intimação em andamento"}</p><p className="mt-2 font-semibold text-white">{type === "dm" ? settings.defaultTitle : "Intimação em andamento"}</p><p className="mt-2 whitespace-pre-wrap text-sm text-zinc-300">{type === "dm" ? settings.defaultText : settings.defaultMessage}</p>{type === "dm" && settings.imageUrl ? <img src={settings.imageUrl} className="mt-4 max-h-52 w-full rounded object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /> : null}{type === "dm" ? <div className="mt-4 text-xs text-zinc-400"><p><b>Enviado por:</b> Nome do staff</p><p><b>Servidor:</b> {guild.name}</p><p><b>Data/Horário:</b> automático</p></div> : null}{type === "dm" && dmButton?.label && dmButton?.url ? <button className="mt-4 rounded bg-[#5865f2] px-3 py-2 text-sm text-white">{dmButton.label}</button> : null}{type === "dm" && settings.footerText ? <p className="mt-4 text-xs text-zinc-500">{settings.footerText}</p> : null}{type === "summons" ? <button className="mt-4 rounded bg-red-600 px-3 py-2 text-sm text-white">Finalizar Intimação</button> : null}</div></div></CardContent></Card>
    </div>
